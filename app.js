...
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InhtdHhidGNleHlydmVpZndmc3diIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODQ5Njc4MDYsImV4cCI6MjEwMDU0MzgwNn0.eVW5nAZeKu8NoryZUYRKsmHGoHrPV719X3cRINCUkjk';
...
function showPhotoLightbox(url, name) {
  ...
  overlay.style.cssText = 'position:fixed;inset:0;z-index:1000;background:rgba(6,13,7,0.92);display:flex;align-items:center;justify-content:center;padding:24px;cursor:zoom-out';
  overlay.innerHTML = `
...
// ── STRIPE CONFIG ──────────────────────────────────────────────────────────
const STRIPE_PK = 'pk_test_51TxoKTFXkOXN7EUyLw4pgbOi0oIBtV19KQocWnnmPnICXEP8nHxfxaMSj63Cl8Y99eGhOEuo7YNQ9ZJyNv988YS6000Mp34367';
const STRIPE_PRICES = {
...
async function getSupabaseToken() {
  ...
  try {
    ...
    if ((session.expires_at || 0) - 60 < Date.now() / 1000 && session.refresh_token) {
...
const SB = {
  ...
  async deleteSerpente(id) {
    return this.req(`serpenti?id=eq.${id}`, { method:'DELETE', prefer:'return=minimal' });
  },

  // VENDUTI
  // Lista/grafico: esclude logs_snapshot (~85-90% del peso di ogni riga) — serve solo per il dettaglio.
  async getVenduti() {
    return this.req('venduti?select=id,snake_id,nome,specie,morfo,sesso,nascita,peso,provenienza,icd,note,data_vendita,acquirente,prezzo,note_vendita,snake_created_at,created_at,user_id&order=data_vendita.desc');
  },
  ...
  async getRecentLogs(limit) {
    return this.req(`logs?select=*&order=data.desc,created_at.desc&limit=${limit}`);
  },
  ...
  async countLogs(filters={}) {
    ...
    const token = await getSupabaseToken();
    const res = await fetch(`${SUPABASE_URL}/rest/v1/logs?select=id${qs ? '&'+qs : ''}`, {
      method: 'HEAD',
      headers: {
        'apikey': SUPABASE_KEY,
        'Authorization': `Bearer ${token}`,
  ...
  async insertLogsBulk(rows) {
    ...
    return this.req('logs', { method:'POST', body: JSON.stringify(rows.map(normalizeLogRow)) });
  },
  async deleteLog(id) {
    return this.req(`logs?id=eq.${id}`, { method:'DELETE', prefer:'return=minimal' });
  },
  async updateSerpente(id, data) {
    return this.req(`serpenti?id=eq.${id}`, { method:'PATCH', body: JSON.stringify(data), prefer:'return=minimal' });
  },
...
function _refreshLastLogsForSnake(snakeId) {
  ...
  Object.keys(_lastLogsPerSnake).forEach(k => { if (k.startsWith(snakeId + '|')) delete _lastLogsPerSnake[k]; });
  logs.forEach(l => {
...
function removeLogFromCaches(logId, snakeId) {
  if (_logsCache[snakeId]) _logsCache[snakeId] = _logsCache[snakeId].filter(l => l.id !== logId);
...
const LOG_FIELDS = ['id','snake_id','user_id','tipo','data','grammi','qty','food_tipo','feci_tipo','num_uova','fertili','temp','note','pulizia_tipo','partner_id','partner_esterno','created_at'];
function normalizeLogRow(l) {
  const row = {};
  LOG_FIELDS.forEach(f => { row[f] = (f in l && l[f] !== undefined) ? l[f] : null; });
  return row;
}
...
function addLogsToCaches(snakeId, logs) {
  ...
  _recentLogs = _recentLogs.concat(logs)
...
async function loadAll() {
  ...
  if (navigator.onLine) { try { await replayOfflineQueue(); } catch(e) { console.warn('[Sync] replay all\'avvio fallito:', e); } }
  ...
  _logsCache = {};
  _lastLogsPerSnake = {};
  (results[3].status === 'fulfilled' ? (results[3].value || []) : []).forEach(l => {
...
function setOnline(val) {
  ...
  if (mob) mob.style.background = val ? 'var(--accent-lime)' : 'var(--accent-red)';
}
...
function logsForSnake(snakeId) { return _logsCache[snakeId] || []; }
...
function refreshDrawerSnakes() {
  ...
  if (!_snakes.length) html += '<div style="padding:8px 12px;font-size:12px;color:var(--text-dim)">Nessun serpente</div>';
...
function refreshSidebar() {
  ...
  const SIDEBAR_LIMIT = 5;
  _snakes.slice(0, SIDEBAR_LIMIT).forEach(s => {
    const active = currentPage==='dettaglio' && currentSnakeId===s.id ? 'active' : '';
...
function renderDashboard() {
  ...
  const foodMonth = _recentLogs.filter(l=>l.tipo==='cibo' && l.data && l.data.startsWith(thisMonth)).length;
  let eggsTotal = _eggsTotalCache;
  if (eggsTotal === null) {
    SB.countLogs({ tipo: 'uova' }).then(n => { _eggsTotalCache = n; if (currentPage==='dashboard') renderDashboard(); }).catch(()=>{ _eggsTotalCache = 0; });
    eggsTotal = '…';
  }
  ...
  _snakes.forEach(s => {
    ...
    const last = _recentLogs.filter(l=>l.snake_id===s.id && l.tipo==='cibo').sort((a,b)=>b.data.localeCompare(a.data))[0];
    if (last) {
      const days = Math.floor((Date.now()-new Date(last.data))/86400000);
      if (days>=14) warnings.push({nome:s.nome, days, id:s.id});
    } else { warnings.push({nome:s.nome, days:null, id:s.id}); }
  });

  const chartData = [];
  for (let i=6;i>=0;i--) {
    const d = new Date(Date.now()-i*86400000).toISOString().split('T')[0];
    chartData.push(_recentLogs.filter(l=>l.data===d && l.tipo==='cibo').length);
  }
  ...
  <div class="dash-stats-grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:12px;margin-bottom:20px">
    ${[['🐍',_snakes.length,t('dashboard_totale'),'var(--accent-gold)'],['♂️',totM,t('dashboard_maschi'),'#5b9bd5'],
       ['♀️',totF,t('dashboard_femmine'),'var(--accent-pink)'],['🥩',foodMonth,t('dashboard_pasti_mese'),'var(--accent-lime)'],
  ...
  <div class="dash-bottom-grid" style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
    <div class="card">
      ...
      <div style="display:flex;justify-content:space-between;margin-top:6px;font-size:9px;color:var(--text-dim)">
        ${['6','5','4','3','2'].map(n=>`<span>${n}${t('dashboard_giorni_fa')}</span>`).concat([`<span>${t('dashboard_ieri')}</span>`,`<span>${t('dashboard_oggi')}</span>`]).join('')}
      </div>
    </div>
    <div class="card">
...
function renderSerpenti() {
  ...
  if (!_snakes.length) {
    ...
    html += '<div class="snakes-grid">' + _snakes.map(s => {
...
async function saveSerpente() {
  ...
  if (!nome) { toast(t('err_nome_richiesto'),'#c0392b'); return; }
  if (!sesso) { toast(t('err_sesso_richiesto'),'#c0392b'); return; }
  if (isNameTaken(nome)) { toast(`"${nome}" ${t('err_nome_usato')}`, '#c0392b'); return; }
  if (!canAddSnake()) { showUpgradeModal(); return; }
  ...
  const snake = {
    ...
    foto_position: _addFotoData ? (document.getElementById('foto-position-input')?.value || '50% 50%') : '50% 50%',
    ...readParentFields(),
...
function renderDettaglio() {
  ...
  if (!snake) { showPage('serpenti'); return; }
  ...
  if (!_logsCache[snake.id]) {
    ...
    SB.getLogs(snake.id).then(rows => {
      _logsCache[snake.id] = rows || [];
      if (currentPage === 'dettaglio' && currentSnakeId === snake.id) renderDettaglio();
    }).catch(e => toast('Errore: ' + e.message, '#c0392b'));
    return;
  }

  const logs = logsForSnake(snake.id);
  const foodLogs = logs.filter(l=>l.tipo==='cibo').sort((a,b)=>b.data.localeCompare(a.data));
  const feciLogs = logs.filter(l=>l.tipo==='feci').sort((a,b)=>b.data.localeCompare(a.data));
  const eggsLogs = logs.filter(l=>l.tipo==='uova').sort((a,b)=>b.data.localeCompare(a.data));
  const puliziaLogs = logs.filter(l=>l.tipo==='pulizia').sort((a,b)=>b.data.localeCompare(a.data));
  const mutaLogs = logs.filter(l=>l.tipo==='muta').sort((a,b)=>b.data.localeCompare(a.data));
  const pesoLogs = logs.filter(l=>l.tipo==='peso').sort((a,b)=>a.data.localeCompare(b.data));
  const totalFood = foodLogs.reduce((s,l)=>s+(parseFloat(l.grammi)||0),0);
  ...
  <!-- TAB UOVA -->
  ${snake.sesso==='F'?`<div id="tab-content-uova" style="display:none">
    <div class="card" style="margin-bottom:14px">
  ...
  <div id="tab-content-pulizia" style="display:none">
    ...
    <div class="card">
      ...
      ${puliziaLogs.length===0?`<div style="color:var(--text-dim);font-size:14px;padding:10px 0">${t('nessuna_pulizia')}</div>`:
        puliziaLogs.map(l=>`<div class="log-entry">
  ...
  <div id="tab-content-muta" style="display:none">
    ...
    <div class="card">
      ...
      ${mutaLogs.length===0?`<div style="color:var(--text-dim);font-size:14px;padding:10px 0">${t('nessuna_muta')}</div>`:
        mutaLogs.map(l=>`<div class="log-entry">
  ...
  <div id="tab-content-peso" style="display:none">
    ...
    ${pesoLogs.length > 0 ? `<div class="card" style="margin-bottom:14px">
      ...
      <div style="background:var(--bg-moss);border-radius:8px;padding:16px 8px;overflow-x:auto">
        ${(()=>{
          ...
          if(pl.length<2) return `<div style="color:var(--text-dim);font-size:13px;text-align:center;padding:40px 0">${t('servono_2_pesate')}</div>`;
          const vals=pl.map(l=>l.grammi);
          const minV=Math.min(...vals), maxV=Math.max(...vals);
          const range=(maxV-minV)||1;
          const pointSpacing=76;
          const leftPad=36, rightPad=36;
          const chartW=Math.max(400, (pl.length-1)*pointSpacing + leftPad + rightPad);
          const plotH=150, topPad=26, dateAreaH=30;
          const svgH=topPad+plotH+dateAreaH;
          const xForI=i=>leftPad+i*pointSpacing;
          const yForV=v=>topPad+plotH-((v-minV)/range)*plotH;
          const shortDate=d=>{ if(!d) return ''; const [y,m,dd]=d.split('-'); return `${dd}/${m}/${y.slice(2)}`; };
          const points=pl.map((l,i)=>[xForI(i), yForV(l.grammi), l.grammi, l.data]);
    ...
    <div class="card">
      ...
      ${pesoLogs.length===0?`<div style="color:var(--text-dim);font-size:14px;padding:10px 0">${t('nessuna_pesata')}</div>`:
        [...pesoLogs].reverse().map(l=>`<div class="log-entry">
    ...
    <div class="card">
      ...
      <div id="info-view-mode">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:12px">
          ${[[t('lbl_nome'),snake.nome],[t('lbl_specie'),snake.specie||'—'],[t('lbl_morfo'),snake.morfo||'—'],
             [t('lbl_sesso'),snake.sesso==='M'?'♂ '+t('aggiungi_maschio'):'♀ '+t('aggiungi_femmina')],
             [t('lbl_nascita'),fmtDate(snake.nascita)],[t('serpenti_eta'),age(snake.nascita)],
             [t('lbl_peso'),snake.peso?snake.peso+'g':'—'],[t('lbl_provenienza'),snake.provenienza||'—'],
             [t('lbl_icd'),snake.icd]
          ].map(([k,v])=>`
        ...
        ${snake.note?`<div style="margin-top:12px;background:var(--bg-moss);border-radius:8px;padding:12px">
...
function setFotoPosition(pos) {
  ...
  if (editPreview && editPreview.tagName === 'IMG') editPreview.style.objectPosition = pos;
  if (addImg) addImg.style.objectPosition = pos;
  document.querySelectorAll('.pos-picker-btn').forEach(b => {
    b.style.background = b.dataset.pos === pos ? 'var(--accent-gold)' : 'var(--bg-moss)';
  });
}

// ── GENEALOGIA: campo genitore (select tra archivio + opzione esterna) ─────
function renderParentField(role, currentId, currentEsterno, sessoFiltro, excludeId) {
  const label = role === 'padre' ? t('gen_padre') : role === 'madre' ? t('gen_madre') : t('gen_partner');
  const inputId = role === 'padre' ? 'f-padre-esterno' : role === 'madre' ? 'f-madre-esterna' : 'eggs-partner-esterno';
  const selectId = role === 'partner' ? 'eggs-partner-select' : `f-${role}-select`;
  const isEsterno = !currentId && !!currentEsterno;
  const options = _snakes.filter(s => s.sesso === sessoFiltro && s.id !== excludeId);
...
function toggleParentExterno(role) {
  ...
  if (select && input) input.style.display = select.value === '__esterno__' ? 'block' : 'none';
}
...
function readParentFields() {
  ...
  return {
    ...
    padre_esterno: padreSel === '__esterno__' ? (document.getElementById('f-padre-esterno')?.value.trim() || null) : null,
    madre_id: (madreSel && madreSel !== '__esterno__') ? madreSel : null,
    madre_esterna: madreSel === '__esterno__' ? (document.getElementById('f-madre-esterna')?.value.trim() || null) : null,
  };
}

// ── GENEALOGIA: sezione figli / fratelli di cova nella scheda dettaglio ────
function renderGenealogySection(snake) {
  const figli = _snakes.filter(s => s.padre_id === snake.id || s.madre_id === snake.id);
  const fratelli = (snake.padre_id || snake.madre_id)
    ? _snakes.filter(s => s.id !== snake.id &&
        ((snake.padre_id && s.padre_id === snake.padre_id) || (snake.madre_id && s.madre_id === snake.madre_id)))
    : [];
  const hasParents = snake.padre_id || snake.padre_esterno || snake.madre_id || snake.madre_esterna || snake.genetica;
  if (!hasParents && figli.length === 0 && fratelli.length === 0) return '';

  const snakeChip = s => `<span onclick="showPage('dettaglio','${s.id}')" style="display:inline-flex;align-items:center;gap:5px;background:var(--bg-moss);border:1px solid var(--border);border-radius:20px;padding:4px 10px 4px 4px;font-size:12px;cursor:pointer;margin:3px 4px 3px 0">
  ...
  return `<div class="card" style="margin-bottom:14px">
    <div class="card-title">🧬 ${t('gen_title')}</div>
    ${hasParents ? `<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:12px;margin-bottom:${(figli.length || fratelli.length) ? '14px' : '0'}">
      <div style="background:var(--bg-moss);border-radius:8px;padding:10px">
        <div style="font-size:9px;color:var(--text-dim);text-transform:uppercase;letter-spacing:1px;margin-bottom:2px">${t('gen_padre')}</div>
        ${snake.padre_id && _snakes.find(s => s.id === snake.padre_id) ? parentChip(_snakes.find(s => s.id === snake.padre_id)) :
          snake.padre_esterno ? `<div style="font-size:13px;color:var(--text-bright);margin-top:3px">${esc(snake.padre_esterno)} <span style="font-size:10px;color:var(--text-dim)">(${t('gen_esterno_badge')})</span></div>` :
      ...
      <div style="background:var(--bg-moss);border-radius:8px;padding:10px">
        ...
        ${snake.madre_id && _snakes.find(s => s.id === snake.madre_id) ? parentChip(_snakes.find(s => s.id === snake.madre_id)) :
          snake.madre_esterna ? `<div style="font-size:13px;color:var(--text-bright);margin-top:3px">${esc(snake.madre_esterna)} <span style="font-size:10px;color:var(--text-dim)">(${t('gen_esterno_badge')})</span></div>` :
          `<div style="font-size:13px;color:var(--text-dim);margin-top:3px">${t('gen_non_impostato')}</div>`}
      </div>
      ${snake.genetica ? `<div style="background:var(--bg-moss);border-radius:8px;padding:10px;grid-column:1/-1">
...
function renderLogList(logs, type, snakeId) {
  const icons = { food:'🥩', feci:'💩', eggs:'🥚', pulizia:'🧹', muta:'🦎', peso:'⚖️' };
  if (!logs.length) return `<div style="color:var(--text-dim);font-size:14px;padding:10px 0">${t('nessuna_registrazione')}</div>`;
  return logs.map(l => {
    let desc = '';
    if (l.tipo==='cibo') desc=`${l.food_tipo||t('log_cibo_default')} ×${l.qty||1} — ${l.grammi||0}g`;
    if (l.tipo==='feci') desc=`${l.feci_tipo||'—'}`;
    if (l.tipo==='uova') {
      const partnerNome = l.partner_id ? (_snakes.find(s=>s.id===l.partner_id)?.nome || null) : (l.partner_esterno || null);
...
async function logFood(snakeId) {
  ...
  try {
    ...
    renderDettaglio();
  } catch(e) { toast('Errore: '+e.message,'#c0392b'); btn.disabled=false; btn.innerHTML='💾 Salva Pasto'; }
}
...
async function logFeci(snakeId) {
  ...
  const log={
    id:'L'+Date.now()+Math.random().toString(36).slice(2,5),
    snake_id:snakeId, tipo:'feci', data,
  ...
  try {
    ...
    renderDettaglio(); switchTab('feci');
  } catch(e) { toast('Errore: '+e.message,'#c0392b'); btn.disabled=false; btn.innerHTML='💾 Salva'; }
}
...
async function logUova(snakeId) {
  ...
  const log={
    id:'L'+Date.now()+Math.random().toString(36).slice(2,5),
    snake_id:snakeId, tipo:'uova', data,
  ...
  const partnerSel = document.getElementById('eggs-partner-select')?.value || '';
  log.partner_id = (partnerSel && partnerSel !== '__esterno__') ? partnerSel : null;
  log.partner_esterno = partnerSel === '__esterno__' ? (document.getElementById('eggs-partner-esterno')?.value.trim() || null) : null;
  try {
    await SB.insertLog(log);
    addLogToCaches(log);
    toast('🥚 Deposizione salvata nel cloud!');
    renderDettaglio(); switchTab('uova');
  } catch(e) { toast('Errore: '+e.message,'#c0392b'); btn.disabled=false; btn.innerHTML='💾 Salva'; }
}
...
async function logPulizia(snakeId) {
  ...
  const log={
    id:'L'+Date.now()+Math.random().toString(36).slice(2,5),
    snake_id:snakeId, tipo:'pulizia', data,
  ...
  try {
    ...
    renderDettaglio(); switchTab('pulizia');
  } catch(e) { toast('Errore: '+e.message,'#c0392b'); btn.disabled=false; btn.innerHTML='💾 Salva'; }
}
...
function editLogEntry(logId, snakeId, tipo) {
  ...
  const tipoLabels = {cibo:'🥩 Pasto',feci:'💩 Feci',uova:'🥚 Deposizione',muta:'🦎 Muta',peso:'⚖️ Peso',pulizia:'🧹 Pulizia'};
  modal.innerHTML = `
...
async function saveEditLog(logId, snakeId, tipo) {
  ...
  btn.disabled = true; btn.innerHTML = '<span class="spinner"></span>';
  const updates = { data: document.getElementById('editlog-data').value, note: document.getElementById('editlog-note').value || null };
  if (tipo==='cibo') {
    updates.food_tipo = document.getElementById('editlog-food-tipo').value;
    updates.grammi = parseFloat(document.getElementById('editlog-grammi').value) || null;
    ...
    updates.num_uova = parseInt(document.getElementById('editlog-num-uova').value) || null;
    updates.fertili = parseInt(document.getElementById('editlog-fertili').value) || null;
    updates.temp = parseFloat(document.getElementById('editlog-temp').value) || null;
  } else if (tipo==='muta') {
    updates.feci_tipo = document.getElementById('editlog-muta-tipo').value;
  } else if (tipo==='peso') {
    updates.grammi = parseFloat(document.getElementById('editlog-grammi').value) || null;
  } else if (tipo==='pulizia') {
    updates.pulizia_tipo = document.getElementById('editlog-pulizia-tipo').value;
  }
  try {
...
async function logMuta(snakeId) {
  ...
  const log={
    id:'L'+Date.now()+Math.random().toString(36).slice(2,5),
    snake_id:snakeId, tipo:'muta', data,
  ...
  try {
    ...
    renderDettaglio(); switchTab('muta');
  } catch(e) { toast('Errore: '+e.message,'#c0392b'); btn.disabled=false; btn.innerHTML='💾 Salva Muta'; }
}
...
async function logPeso(snakeId) {
  ...
  if(!data){toast(t('err_data_richiesta'),'#c0392b');return;}
  if(!grammi){toast('Inserisci il peso!','#c0392b');return;}
  const btn=document.getElementById('btn-log-peso');
  btn.disabled=true; btn.innerHTML='<span class="spinner"></span>';
  const log={
    id:'L'+Date.now()+Math.random().toString(36).slice(2,5),
    snake_id:snakeId, tipo:'peso', data, grammi,
  ...
  try {
    ...
    if(snake) { snake.peso = grammi; await SB.updateSerpente(snakeId, {peso: grammi}); }
    toast('⚖️ Peso registrato!');
    renderDettaglio(); switchTab('peso');
  } catch(e) { toast('Errore: '+e.message,'#c0392b'); btn.disabled=false; btn.innerHTML='💾 Salva Peso'; }
}
...
async function uploadLogoFromPdf(file) {
  try {
    ...
    if (typeof pdfjsLib === 'undefined') throw new Error('Libreria PDF non disponibile, ricarica la pagina');
    ...
    const logoData = centerOnSquareCanvas(renderCanvas, renderCanvas.width, renderCanvas.height);
    await saveLogoToServer(logoData);
    _userLogo = logoData;
    renderProfilo();
...
async function removeLogo() {
  if (!confirm('Rimuovere il logo? Non comparirà più su etichette e PDF.')) return;
  try {
...
async function changeAppLang(lang) {
  ...
  try {
    ...
    toast(lang === 'it' ? '🌐 Lingua impostata su Italiano' : '🌐 Language set to English');
  } catch(err) {
...
async function saveEditSerpente(snakeId) {
  ...
  const updates = {
    ...
    provenienza: document.getElementById('edit-provenienza').value.trim() || null,
  ...
  if (!updates.nome) { toast('Il nome è obbligatorio!','#c0392b'); btn.disabled=false; btn.innerHTML='💾 Salva Modifiche'; return; }
  if (isNameTaken(updates.nome, snakeId)) { toast(`"${updates.nome}" già utilizzato`, '#c0392b'); btn.disabled=false; btn.innerHTML='💾 Salva Modifiche'; return; }
  try {
    ...
    toast('Errore: '+e.message,'#c0392b');
    btn.disabled=false; btn.innerHTML='💾 Salva Modifiche';
  }
}

async function deleteSerpente(id) {
  if(!confirm('Eliminare questo serpente e tutte le sue registrazioni?\nI dati verranno cancellati dal database.')) return;
  try {
...
async function confermaVendita(snakeId) {
  ...
  if (!data) { toast('Inserisci la data vendita!', '#c0392b'); return; }
  ...
  try {
    ...
    const venduto = {
      id: 'V' + Date.now() + Math.random().toString(36).slice(2,5).toUpperCase(),
      snake_id: snake.id,
      nome: snake.nome,
      specie: snake.specie,
      morfo: snake.morfo,
    ...
    toast('💰 ' + snake.nome + ' segnato come venduto con ' + logsSnapshot.length + ' log salvati!');
    showPage('venduti');
  } catch(e) {
    toast('Errore: ' + e.message, '#c0392b');
    btn.disabled = false; btn.innerHTML = '💰 Conferma Vendita';
  }
}
...
//  PAGINA VENDUTI
// ═══════════════════════════════════════
const MESI_ABBR = { it: ['Gen','Feb','Mar','Apr','Mag','Giu','Lug','Ago','Set','Ott','Nov','Dic'], en: ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'] };
const MESI_FULL = { it: ['Gennaio','Febbraio','Marzo','Aprile','Maggio','Giugno','Luglio','Agosto','Settembre','Ottobre','Novembre','Dicembre'], en: ['January','February','March','April','May','June','July','August','September','October','November','December'] };

function getVendutiYears() {
  const years = new Set();
  _venduti.forEach(v => { if (v.data_vendita) years.add(v.data_vendita.slice(0, 4)); });
...
function renderVenduti() {
  ...
  const filteredIncasso = filtered.reduce((s, v) => s + (parseFloat(v.prezzo) || 0), 0);
  const years = getVendutiYears();
  const mesiAbbr = MESI_ABBR[_appLang] || MESI_ABBR.it;
  const mesiFull = MESI_FULL[_appLang] || MESI_FULL.it;
  const filtriAttivi = _vendutiFilterYear !== 'all' || _vendutiFilterMonth !== 'all';

  // ── Costruzione barre del grafico ──
  let chartBars = [];
  if (_vendutiFilterYear === 'all') {
    chartBars = [...years].sort((a, b) => a.localeCompare(b)).map(y => {
      const inYear = _venduti.filter(v => v.data_vendita && v.data_vendita.slice(0, 4) === y);
      const incasso = inYear.reduce((s, v) => s + (parseFloat(v.prezzo) || 0), 0);
      return { label: y, count: inYear.length, incasso, onclick: `setVendutiYear('${y}')`, active: false };
    });
  } else {
    chartBars = mesiAbbr.map((label, idx) => {
      const inMonth = _venduti.filter(v => v.data_vendita && v.data_vendita.slice(0, 4) === _vendutiFilterYear && (parseInt(v.data_vendita.slice(5, 7), 10) - 1) === idx);
      const incasso = inMonth.reduce((s, v) => s + (parseFloat(v.prezzo) || 0), 0);
      const active = _vendutiFilterMonth !== 'all' && parseInt(_vendutiFilterMonth, 10) === idx;
      return { label, count: inMonth.length, incasso, onclick: `setVendutiMonth(${idx})`, active };
    });
  }
  ...
  if (years.length > 0) {
    ...
    const fmtEuroAxis = (val) => val >= 1000 ? '€' + (val / 1000).toFixed(1).replace('.0', '') + 'k' : '€' + Math.round(val);
    const yLabels = [];
  ...
  if (!filtered.length) {
    ...
    html += '<div class="snakes-grid">' + filtered.map(v => `
...
async function annullaVendita(vendutoId) {
  ...
  try {
    ...
    const restored = {
      id: v.snake_id,
      nome: v.nome, specie: v.specie, morfo: v.morfo, sesso: v.sesso,
    ...
    const logsToRestore = (v.logs_snapshot || []).map(l => ({ ...l, snake_id: restored.id }));
...
function renderDettaglioVenduto() {
  ...
  if (!v.logs_snapshot) {
    document.getElementById('main-content').innerHTML = `
      ...
      <div class="empty-state"><div class="empty-icon">⏳</div><h3>Caricamento storico…</h3></div>`;
    SB.getVendutoFull(vendutoId).then(full => {
      if (full) Object.assign(v, full);
      if (currentPage === 'dettaglioVenduto' && currentSnakeId === vendutoId) renderDettaglioVenduto();
    }).catch(e => toast('Errore: ' + e.message, '#c0392b'));
    return;
  }
  ...
  const giorniAllevamento = dataIngresso ? Math.floor((dataVendita - dataIngresso) / 86400000) : null;
  ...
  <div id="tab-content-cibo">
    <div class="card">
      ...
      ${foodLogs.length===0?`<div style="color:var(--text-dim);font-size:14px;padding:10px 0">${t('dv_nessun_pasto')}</div>`:
        foodLogs.map(l=>`<div class="log-entry">
  ...
  <div id="tab-content-feci" style="display:none">
    <div class="card">
      ...
      ${feciLogs.length===0?`<div style="color:var(--text-dim);font-size:14px;padding:10px 0">${t('nessuna_registrazione')}</div>`:
        feciLogs.map(l=>`<div class="log-entry">
  ...
  <!-- UOVA -->
  ${v.sesso==='F'?`<div id="tab-content-uova" style="display:none">
    <div class="card">
      <div class="card-title">🥚 ${t('dv_storico_deposizioni')}</div>
      ${eggsLogs.length===0?`<div style="color:var(--text-dim);font-size:14px;padding:10px 0">${t('dv_nessuna_deposizione')}</div>`:
        eggsLogs.map(l=>`<div class="log-entry">
  ...
  <div id="tab-content-info" style="display:none">
    <div class="card">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:12px">
        ${[[t('lbl_nome'),v.nome],[t('lbl_specie'),v.specie||'—'],[t('lbl_morfo'),v.morfo||'—'],
      ...
      ${v.note?`<div style="margin-top:12px;background:var(--bg-moss);border-radius:8px;padding:12px">
        <div style="font-size:9px;color:var(--text-dim);text-transform:uppercase;letter-spacing:1px;margin-bottom:4px">${t('label_note')}</div>
...
async function printResoconto(vendutoId) {
  ...
  if (!v.logs_snapshot) {
    win.document.body.style.cssText = 'font-family:sans-serif;padding:40px;color:#333';
    win.document.body.textContent = 'Caricamento…';
    try {
      const full = await SB.getVendutoFull(vendutoId);
      if (full) Object.assign(v, full);
    } catch(e) {
      win.document.body.textContent = 'Errore nel caricamento dei dati: ' + e.message;
      return;
    }
  }
  ...
  const logs = v.logs_snapshot || [];
  const foodLogs = logs.filter(l => l.tipo === 'cibo').sort((a,b) => a.data.localeCompare(b.data));
  const feciLogs = logs.filter(l => l.tipo === 'feci').sort((a,b) => a.data.localeCompare(b.data));
  const eggsLogs = logs.filter(l => l.tipo === 'uova').sort((a,b) => a.data.localeCompare(b.data));
  const totalCibo = foodLogs.reduce((s,l) => s + (parseFloat(l.grammi) || 0), 0);
...
function renderRegistro() {
  const logs = [..._recentLogs].sort((a,b)=>b.data.localeCompare(a.data)).slice(0,80);
  const snakeMap = {}; _snakes.forEach(s=>snakeMap[s.id]=s);
  const icons={cibo:'🥩',feci:'💩',uova:'🥚',pulizia:'🧹'};
  const types={cibo:'food',feci:'feci',uova:'eggs',pulizia:'pulizia'};
  ...
  if(!logs.length) {
    ...
    logs.forEach(l=>{
      ...
      let desc='';
      if(l.tipo==='cibo') desc=`${l.food_tipo||t('log_cibo_default')} ×${l.qty||1} — ${l.grammi||0}g`;
      if(l.tipo==='feci') desc=`${t('log_feci_prefix')}${l.feci_tipo||'—'}`;
      if(l.tipo==='uova') desc=`${t('log_deposizione_prefix')}${l.num_uova||0} ${t('log_uova_desc')}`;
      if(l.tipo==='pulizia') desc=`${t('log_pulizia_prefix')}${l.pulizia_tipo||'—'}`;
...
function printSchedaSerpente(snakeId) {
  ...
  const foodLogs = logs.filter(l=>l.tipo==='cibo').sort((a,b)=>a.data.localeCompare(b.data));
  const feciLogs = logs.filter(l=>l.tipo==='feci').sort((a,b)=>a.data.localeCompare(b.data));
  const mutaLogs = logs.filter(l=>l.tipo==='muta').sort((a,b)=>a.data.localeCompare(b.data));
  const pesoLogs = logs.filter(l=>l.tipo==='peso').sort((a,b)=>a.data.localeCompare(b.data));
  const eggsLogs = logs.filter(l=>l.tipo==='uova').sort((a,b)=>a.data.localeCompare(b.data));
  const totalCibo = foodLogs.reduce((s,l)=>s+(parseFloat(l.grammi)||0),0);
  ...
  win.document.write(`<!DOCTYPE html><html lang="${_appLang}"><head><meta charset="UTF-8">
...
<div class="info-grid">
  ${[[t('lbl_nome'),snake.nome],[t('lbl_specie'),snake.specie||'—'],[t('lbl_morfo'),snake.morfo||'—'],
     [t('lbl_sesso'),snake.sesso==='M'?'♂ '+t('aggiungi_maschio'):'♀ '+t('aggiungi_femmina')],
     [t('lbl_data_nascita'),snake.nascita?new Date(snake.nascita).toLocaleDateString(dateLocale):'—'],
     [t('serpenti_eta'),age(snake.nascita)],
     [t('pdf_peso_attuale'),snake.peso?snake.peso+'g':'—'],[t('lbl_provenienza'),snake.provenienza||'—'],
     [t('lbl_icd'),snake.icd],
     [t('gen_padre'), (()=>{ const p=snake.padre_id?_snakes.find(s=>s.id===snake.padre_id):null; if(p){const st=[p.specie,p.morfo].filter(Boolean).join(', '); return p.nome+(st?` (${st})`:'');} return snake.padre_esterno||'—'; })()],
     [t('gen_madre'), (()=>{ const m=snake.madre_id?_snakes.find(s=>s.id===snake.madre_id):null; if(m){const st=[m.specie,m.morfo].filter(Boolean).join(', '); return m.nome+(st?` (${st})`:'');} return snake.madre_esterna||'—'; })()],
     [t('gen_genetica'), snake.genetica || '—']
  ].map(([k,v])=>`<div class="info-item"><div class="info-label">${k}</div><div class="info-value">${esc(v)}</div></div>`).join('')}
</div>

<div class="stats-row">
  ${[[t('pdf_pasti_tot'),foodLogs.length],[t('det_cibo_tot'),totalCibo.toFixed(0)+'g'],[t('det_feci'),feciLogs.length],
     [t('det_mute'),mutaLogs.length],[t('pdf_pesate'),pesoLogs.length],
     ...(snake.sesso==='F'?[[t('pdf_deposizioni'),eggsLogs.length]]:[])
  ].map(([l,v])=>`<div class="stat-box"><div class="stat-num">${v}</div><div class="stat-lbl">${l}</div></div>`).join('')}
</div>
...
${makeTable(t('pdf_storico_pasti'),'🥩', foodLogs.map(l=>[new Date(l.data).toLocaleDateString(dateLocale),esc(l.food_tipo)||t('log_cibo_default'),(l.grammi||0)+'g','×'+(l.qty||1),esc(l.note)||'']), [t('pdf_col_data'),t('pdf_col_tipo'),t('pdf_col_peso'),t('pdf_col_qty'),t('pdf_col_note')])}
${makeTable(t('pdf_storico_mute'),'🦎', mutaLogs.map(l=>[new Date(l.data).toLocaleDateString(dateLocale),esc(l.feci_tipo)||t('muta_completa'),esc(l.note)||'']), [t('pdf_col_data'),t('pdf_col_esito'),t('pdf_col_note')])}
${makeTable(t('pdf_storico_peso'),'⚖️', pesoLogs.map(l=>[new Date(l.data).toLocaleDateString(dateLocale),(l.grammi||0)+'g',esc(l.note)||'']), [t('pdf_col_data'),t('pdf_col_peso'),t('pdf_col_note')])}
${makeTable(t('pdf_storico_feci'),'💩', feciLogs.map(l=>[new Date(l.data).toLocaleDateString(dateLocale),esc(l.feci_tipo)||'—',esc(l.note)||'']), [t('pdf_col_data'),t('pdf_col_tipo'),t('pdf_col_note')])}
${snake.sesso==='F' ? makeTable(t('pdf_deposizioni'),'🥚', eggsLogs.map(l=>[new Date(l.data).toLocaleDateString(dateLocale),(l.num_uova||0),(l.fertili||0),l.temp?l.temp+'°C':'—',esc(l.note)||'']), [t('pdf_col_data'),t('pdf_col_uova'),t('pdf_col_fertili'),t('pdf_col_temp'),t('pdf_col_note')]) : ''}
...
function printLabel(id) {
  ...
  const qrSmallUrl = 'https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=' + encodeURIComponent(url) + '&margin=2&color=0d1f0f&bgcolor=ffffff';
  const qrLargeUrl = 'https://api.qrserver.com/v1/create-qr-code/?size=260x260&data=' + encodeURIComponent(url) + '&margin=4&color=0d1f0f&bgcolor=ffffff';
...
function checkHash() {
  ...
  if(hash.startsWith('#snake=')) {
    ...
    if(_snakes.find(s=>s.id===id)){showPage('dettaglio',id);return true;}
  }
  return false;
}
...
(function(){
  ...
  let pointerId = null, startX = 0, startY = 0, curTranslate = DRAWER_W;
  ...
  function clampWithRubberband(x) {
    ...
    if (x > DRAWER_W) return DRAWER_W + rubberband(x - DRAWER_W, DRAWER_W, 0.55);
    return x;
  }
  ...
  function onMove(e) {
    ...
    if (mode === 'pending') {
      ...
      if (Math.abs(dy) > Math.abs(dx)) { mode = 'idle'; return; } // era uno scroll verticale nel drawer, non un drag
      mode = 'dragging';
      try { drawer.setPointerCapture(pointerId); } catch (err) {}
  ...
  function onUp(e) {
    ...
    if (mode === 'dragging') {
      const shouldOpen = velocity < -300 ? false : velocity > 300 ? true : curTranslate > DRAWER_W / 2;
      settle(shouldOpen);
    }
...
function toggleFaq(i) {
  ...
  document.querySelectorAll('.lp-faq-icon').forEach(ic => ic.textContent = '+');
  if (!isOpen) {
...
function initLandingNav() {
  ...
  landing.addEventListener('scroll', () => {
    nav.style.boxShadow = landing.scrollTop > 50 ? '0 4px 30px rgba(0,0,0,0.4)' : '';
    closeMobileNav();
  });
}
...
function showUpdateBanner() {
  ...
  banner.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:10000;padding:12px 16px;background:rgba(8,15,9,0.97);border-bottom:2px solid var(--border);backdrop-filter:blur(12px);display:flex;align-items:center;justify-content:center;gap:14px;flex-wrap:wrap;font-family:\'Inter\',sans-serif';
  banner.innerHTML = `
...
function isNetworkError(e) {
  return e instanceof TypeError || /failed to fetch|networkerror|network request failed/i.test(e.message || '');
}

const _origInsertLog = SB.insertLog.bind(SB);
SB.insertLog = async function(log) {
  // Assegna sempre il user_id dell'utente corrente
  if (_currentUser && _currentUser.id && !log.user_id) log.user_id = _currentUser.id;
  if (!navigator.onLine) {
...
SB.insertSerpente = async function(s) {
  // Assegna sempre il user_id dell'utente corrente
  if (_currentUser && _currentUser.id && !s.user_id) s.user_id = _currentUser.id;
  if (!navigator.onLine) {
...
SB.insertVenduto = async function(v) {
  // Assegna sempre il user_id dell'utente corrente
  if (_currentUser && _currentUser.id && !v.user_id) v.user_id = _currentUser.id;
...
function showLegalPage(page) {
  ...
  modal.style.cssText = 'position:fixed;inset:0;z-index:999;background:var(--bg-deep);overflow-y:auto;padding:24px';
  const legalLang = (typeof _appLang !== 'undefined' && _appLang) ? _appLang : (typeof _landingLang !== 'undefined' ? _landingLang : 'it');
  const content = page === 'privacy'
    ? (legalLang === 'en' ? getPrivacyContentEN() : getPrivacyContent())
    : (legalLang === 'en' ? getTermsContentEN() : getTermsContent());
  modal.innerHTML = `
...
async function renderProfilo() {
  const planLabels = { free:'🔓 Free', pro:'⭐ Pro', admin:'👑 Admin', free_forever:'🎁 Free Forever', for_life:'♾️ For Life' };
  const planColors = { free:'var(--text-dim)', pro:'var(--accent-lime)', admin:'var(--accent-gold)', free_forever:'var(--accent-pink)', for_life:'var(--accent-lime)' };
  const planDesc = {
...
async function loadSubscriptionInfo() {
  ...
  try {
    ...
    const purchaseStr = info.purchaseDate ? new Date(info.purchaseDate * 1000).toLocaleDateString('it-IT') : '—';
    const renewStr = info.periodEnd ? new Date(info.periodEnd * 1000).toLocaleDateString('it-IT') : null;
    ...
    const dateLabel = info.cancelAtPeriodEnd ? 'Accesso fino al' : 'Prossimo rinnovo';
    ...
    el.innerHTML = `
      <div style="display:grid;gap:8px">
        ...
        ${finalRenewStr ? `<div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid rgba(45,80,48,0.3)">
          <span>${dateLabel}</span>
          <strong style="color:${info.cancelAtPeriodEnd ? 'var(--accent-red)' : 'var(--accent-lime)'}">${finalRenewStr}</strong>
        </div>` : ''}
        ${info.cancelAtPeriodEnd ? `<div style="background:rgba(192,57,43,0.1);border:1px solid rgba(192,57,43,0.3);border-radius:8px;padding:12px;margin-top:4px">
        ...
        </div>` : `<div style="background:${info.refundEligible ? 'rgba(109,181,109,0.1)' : 'rgba(45,80,48,0.2)'};border-radius:8px;padding:12px;margin-top:4px">
          ${info.refundEligible
...
function showCancelSubscriptionModal() {
  ...
  modal.style.cssText = 'position:fixed;inset:0;z-index:600;background:rgba(8,15,9,0.94);display:flex;align-items:center;justify-content:center;padding:24px';
  ...
  if (eligible) {
    ...
    const until = info.periodEnd ? new Date(info.periodEnd * 1000).toLocaleDateString('it-IT') : 'fine periodo';
    explanation = `
...
async function doCancelSubscription() {
  ...
  try {
    ...
    if (data.refunded) {
      toast(`✅ Abbonamento disdetto. Rimborso di €${data.refundAmount.toFixed(2)} in elaborazione (3-5 giorni lavorativi).`, '#6db56d');
      _userPlan = 'free';
      applyPlanUI();
    } else if (!data.immediate && data.accessUntil) {
      const until = new Date(data.accessUntil * 1000).toLocaleDateString('it-IT');
      toast(`✅ Abbonamento disdetto. Mantieni l'accesso Pro fino al ${until}.`, '#c9a84c');
    } else {
...
function isNameTaken(name, excludeId = null) {
  ...
  return _snakes.some(s => s.id !== excludeId && (s.nome || '').trim().toLowerCase() === normalized);
}
...
function showLockedSnakeModal() {
  ...
  modal.style.cssText = 'position:fixed;inset:0;z-index:600;background:rgba(8,15,9,0.92);display:flex;align-items:center;justify-content:center;padding:24px';
  modal.innerHTML = `
...
function applySnakeLockUI(snakeId) {
  ...
  root.querySelectorAll('[id^="btn-log-"], #btn-toggle-edit, #btn-save-edit, .log-delete').forEach(el => {
    el.disabled = true;
  ...
  const banner = document.createElement('div');
  banner.style.cssText = 'margin-bottom:14px;padding:14px 16px;background:rgba(201,168,76,0.08);border:1px solid rgba(201,168,76,0.35);border-radius:10px;display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap';
  banner.innerHTML = `
    <div style="display:flex;align-items:center;gap:10px;min-width:210px;flex:1">
      <div style="font-size:22px">🔒</div>
      <div>
...
async function loadUserPlan() {
  ...
  try {
    ...
    const data = await SB.req(`user_plans?user_id=eq.${_currentUser.id}&select=plan,logo_url,lang,valid_until,trial_fiera_id,trial_code,trial_notified_date`);
...
async function markTrialNotifiedServer() {
  ...
  } catch(e) { console.warn('mark_trial_notified fallito:', e); }
}
...
function showTrialCountdownModal(daysRemaining) {
  ...
  modal.style.cssText = 'position:fixed;inset:0;z-index:650;background:rgba(8,15,9,0.92);display:flex;align-items:center;justify-content:center;padding:24px';
  const dayLabel = daysRemaining === 1 ? t('trial_countdown_day_left') : t('trial_countdown_days_left');
  modal.innerHTML = `
...
async function verifyAndUpdatePlan(sessionId) {
  ...
  try {
    ...
    hidePaymentProcessingBanner();
    toast('⏳ Pagamento in elaborazione. Ricarica tra qualche istante.', '#c9a84c');
  } catch(e) {
...
function showPaymentProcessingBanner() {
  ...
  banner.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:600;background:linear-gradient(135deg,#c9a84c,#e0ba5c);color:#0a1a0a;padding:14px;text-align:center;font-weight:600;font-size:14px;display:flex;align-items:center;justify-content:center;gap:10px;box-shadow:0 4px 20px rgba(0,0,0,0.3)';
  banner.innerHTML = '<span class="spinner" style="border-color:#0a1a0a;border-top-color:transparent"></span> Stiamo attivando il tuo piano...';
  document.body.appendChild(banner);
}
...
function showPaymentSuccessModal(plan) {
  const planNames = { pro: 'Pro', for_life: 'For Life', free_forever: 'Free Forever' };
  const modal = document.createElement('div');
  modal.id = 'payment-success-modal';
  modal.style.cssText = 'position:fixed;inset:0;z-index:700;background:rgba(8,15,9,0.95);display:flex;align-items:center;justify-content:center;padding:24px';
  modal.innerHTML = `
  ...
  if (!document.getElementById('success-anim-style')) {
    ...
    style.textContent = '@keyframes successPop{from{transform:scale(.8);opacity:0}to{transform:scale(1);opacity:1}}@keyframes successBounce{0%{transform:scale(0)}60%{transform:scale(1.2)}100%{transform:scale(1)}}';
...
function applyPlanUI() {
  ...
  if (adminMnavSection) adminMnavSection.style.display = isAdmin() ? '' : 'none';
...
function showUpgradeModal() {
  loadScriptOnce('https://js.stripe.com/v3/').catch(err => console.warn('[Stripe]', err.message));
  ...
  modal.innerHTML = `
    <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:16px;padding:32px;max-width:400px;width:100%;text-align:center;box-shadow:var(--shadow)">
      ...
      <div style="background:var(--bg-moss);border-radius:12px;padding:14px;margin-bottom:20px;text-align:left">
        ${['🐍 Serpenti illimitati','📊 Statistiche avanzate','📄 Export PDF & Excel','☁️ Backup prioritario','✅ Supporto dedicato'].map(f=>`
          <div style="font-size:13px;color:var(--text-mid);margin:5px 0;display:flex;align-items:center;gap:6px">${f}</div>`).join('')}
      </div>
...
function selectPlan(plan) {
  ...
  if (plan === 'monthly') {
    ...
    forLife.style.background = 'linear-gradient(135deg,rgba(109,181,109,0.1),rgba(201,168,76,0.1))';
...
async function loadAdminData() {
  try {
    ...
    const plansRes = await fetch(`${SUPABASE_URL}/rest/v1/rpc/get_all_user_plans`, {
      method: 'POST',
      headers: {
        'apikey': SUPABASE_KEY,
        'Authorization': `Bearer ${token}`,
    ...
    if (!plansRes.ok) {
      ...
      throw new Error(errBody.message || errBody.hint || `Errore ${plansRes.status} nel caricare gli utenti`);
    }
    const plans = await plansRes.json();

    // Statistiche VERE di tutta la piattaforma (non solo le tue)
    let globalStats = { serpenti_totali: '—', log_totali: '—', venduti_totali: '—', utenti_con_serpenti: '—' };
    try {
      const statsRes = await fetch(`${SUPABASE_URL}/rest/v1/rpc/get_global_app_stats`, {
        method: 'POST',
        headers: {
          'apikey': SUPABASE_KEY,
          'Authorization': `Bearer ${token}`,
    ...
    const statsHtml = `
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:12px;margin-bottom:24px">
        ${[
          ['👑','Admin',plans.filter(p=>p.plan==='admin').length,'var(--accent-gold)'],
          ['⭐','Pro',plans.filter(p=>p.plan==='pro').length,'var(--accent-lime)'],
          ['♾️','For Life',plans.filter(p=>p.plan==='for_life').length,'var(--accent-lime)'],
          ['🎁','Free Forever',plans.filter(p=>p.plan==='free_forever').length,'var(--accent-pink)'],
          ['🔓','Free',plans.filter(p=>p.plan==='free').length,'var(--text-dim)'],
    ...
    const tableHtml = plans.length === 0 ? `
              ...
              ${plans.map(p => `
                <tr style="border-bottom:1px solid rgba(45,80,48,0.4)" id="row-${p.user_id}">
                  ...
                  <td style="padding:12px 8px;text-align:right">
                    ${p.user_id === '${_currentUser.id}' ? `<span style="font-size:11px;color:var(--accent-gold)">👑 Tu (Admin)</span>` : `
    ...
    const statsGlobaliHtml = `
      <div class="card" style="margin-top:16px">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(120px,1fr));gap:10px">
          ${[
            ['🐍', 'Serpenti totali', globalStats.serpenti_totali, 'var(--accent-lime)'],
            ['📋', 'Log totali', globalStats.log_totali, 'var(--accent-gold)'],
            ['💰', 'Venduti totali', globalStats.venduti_totali, 'var(--accent-pink)'],
            ['👥', 'Utenti con serpenti', globalStats.utenti_con_serpenti, 'var(--accent-lime)'],
          ].map(([ico,lab,val,col])=>`
    ...
    document.getElementById('admin-content').innerHTML = statsHtml + tableHtml + addUserHtml + statsGlobaliHtml + `<div id="admin-fiera-section" style="margin-top:16px"></div>`;
    loadAdminFiera(plans);
  } catch(e) {
    document.getElementById('admin-content').innerHTML = `<div class="card" style="color:var(--accent-red)">Errore caricamento: ${e.message}</div>`;
  }
}
...
function renderAdminFiera(eventi, plans) {
  ...
  function fieraStatoReale(ev) {
    ...
    if (ev.data_fine && oggi > ev.data_fine) return { label: '⏱️ SCADUTA', color: 'var(--accent-red)' };
    if (ev.data_inizio && oggi < ev.data_inizio) return { label: '🕓 NON INIZIATA', color: 'var(--accent-gold)' };
    return null;
  }

  const listHtml = eventi.length === 0 ? `
    <p style="color:var(--text-dim);font-size:13px;padding:8px 0">Nessun evento fiera creato ancora.</p>` : eventi.map(ev => {
    const count = plans.filter(p => p.trial_fiera_id === ev.id).length;
...
async function createFieraEvent() {
  ...
  const data_inizio = document.getElementById('fiera-data-inizio').value || null;
  const data_fine = document.getElementById('fiera-data-fine').value || null;
  const codice = document.getElementById('fiera-codice').value.trim().toUpperCase();
  const durata = parseInt(document.getElementById('fiera-durata').value, 10) || 90;

  if (!nome) { toast('Inserisci il nome evento!', '#c0392b'); return; }
  if (!codice || !/^[A-Z0-9]{4,32}$/.test(codice)) { toast('Codice promo non valido (solo lettere/numeri, min 4 caratteri)', '#c0392b'); return; }
  if (data_inizio && data_fine && data_fine < data_inizio) {
    toast('❌ La data di fine non può essere precedente alla data di inizio!', '#c0392b');
    return;
  }

  const btn = document.getElementById('btn-crea-fiera');
  btn.disabled = true; btn.innerHTML = '<span class="spinner"></span> Creazione...';
  try {
    await SB.req('fiere_eventi', {
      method: 'POST',
      body: JSON.stringify({
        nome, luogo: luogo || null, data_inizio, data_fine,
        codice_promo: codice, durata_trial_giorni: durata
      })
    });
    ...
    let msg = e.message;
    if (msg.includes('duplicate') || msg.includes('unique')) msg = 'Codice promo già in uso';
    else if (msg.includes('fiere_eventi_date_range_check')) msg = 'La data di fine non può essere precedente alla data di inizio';
    else if (msg.includes('fiere_eventi_durata_positiva_check')) msg = 'La durata del trial deve essere maggiore di zero';
    toast('❌ ' + msg, '#c0392b');
    btn.disabled = false; btn.innerHTML = '🎪 Crea evento e genera link';
  }
}

async function toggleFieraAttivo(id, current) {
  try {
    await SB.req(`fiere_eventi?id=eq.${id}`, { method:'PATCH', prefer:'return=minimal', body: JSON.stringify({ attivo: !current }) });
...
async function deleteFieraEvent(id, count) {
  if (count > 0) {
    toast('⚠️ Questo evento ha iscritti: non può essere eliminato, disattivalo invece.', '#c0392b');
    return;
  }
  if (!confirm('Eliminare definitivamente questo evento fiera?')) return;
  try {
    await SB.req(`fiere_eventi?id=eq.${id}`, { method:'DELETE', prefer:'return=minimal' });
...
function editFieraEvent(id) {
  ...
  if (!ev) { toast('Evento non trovato, ricarico...', '#c0392b'); loadAdminData(); return; }

  const prev = document.getElementById('fiera-edit-modal');
  if (prev) prev.remove();
  const modal = document.createElement('div');
  modal.id = 'fiera-edit-modal';
  modal.style.cssText = 'position:fixed;inset:0;z-index:1000;background:rgba(8,15,9,0.92);display:flex;align-items:center;justify-content:center;padding:24px;overflow-y:auto';
  modal.innerHTML = `
...
async function saveFieraEdit(id) {
  ...
  const data_inizio = document.getElementById('edit-fiera-data-inizio').value || null;
  const data_fine = document.getElementById('edit-fiera-data-fine').value || null;
  const durata = parseInt(document.getElementById('edit-fiera-durata').value, 10) || 90;
  const codiceUnlocked = document.getElementById('edit-fiera-unlock-codice').checked;
  const codice = document.getElementById('edit-fiera-codice').value.trim().toUpperCase();

  if (!nome) { toast('Il nome evento è obbligatorio!', '#c0392b'); return; }
  if (data_inizio && data_fine && data_fine < data_inizio) {
    toast('❌ La data di fine non può essere precedente alla data di inizio!', '#c0392b');
    return;
  }
  if (codiceUnlocked && (!codice || !/^[A-Z0-9]{4,32}$/.test(codice))) {
    toast('Codice promo non valido (solo lettere/numeri, min 4 caratteri)', '#c0392b');
    return;
  }
  ...
  btn.disabled = true; btn.innerHTML = '<span class="spinner"></span> Salvataggio...';
  try {
    const body = { nome, luogo: luogo || null, data_inizio, data_fine, durata_trial_giorni: durata };
    if (codiceUnlocked) body.codice_promo = codice;
    await SB.req(`fiere_eventi?id=eq.${id}`, { method:'PATCH', prefer:'return=minimal', body: JSON.stringify(body) });
    toast('✅ Evento aggiornato!');
    closeModalEl(document.getElementById('fiera-edit-modal'));
    await loadAdminData();
  } catch(e) {
    let msg = e.message;
    if (msg.includes('duplicate') || msg.includes('unique')) msg = 'Codice promo già in uso da un altro evento';
    else if (msg.includes('fiere_eventi_date_range_check')) msg = 'La data di fine non può essere precedente alla data di inizio';
    else if (msg.includes('fiere_eventi_durata_positiva_check')) msg = 'La durata del trial deve essere maggiore di zero';
    toast('❌ ' + msg, '#c0392b');
    btn.disabled = false; btn.innerHTML = '💾 Salva modifiche';
...
async function showFieraHistory(id, nome) {
  ...
  try {
    const storia = await SB.req(`fiere_eventi_storico?fiera_id=eq.${id}&select=*&order=changed_at.desc`);
    ...
    const campoLabels = { codice_promo:'Codice promo', nome:'Nome', luogo:'Luogo', data_inizio:'Data inizio', data_fine:'Data fine', durata_trial_giorni:'Durata trial (giorni)', attivo:'Stato attivo' };
    contentEl.innerHTML = storia.map(h => `
    ...
    document.getElementById('fiera-history-content').innerHTML = `<div style="color:var(--accent-red);font-size:13px">Errore caricamento: ${e.message}</div>`;
  }
}
...
async function adminAddUser() {
  ...
  if (!uuid) { toast('Inserisci un UUID!','#c0392b'); return; }
  const uuidPattern = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
  if (!uuidPattern.test(uuid)) { toast('UUID non valido!','#c0392b'); return; }
  ...
  try {
    ...
    toast('❌ ' + e.message, '#c0392b');
    btn.disabled = false; btn.innerHTML = '➕ Aggiungi Utente';
  }
}
// ═══════════════════════════════════════
//  AUTH SUPABASE
...
const I18N = {
  it: {
    ...
    dashboard_totale: 'Totale', dashboard_maschi: 'Maschi', dashboard_femmine: 'Femmine',
    dashboard_pasti_mese: 'Pasti/mese', dashboard_deposizioni: 'Deposizioni',
    dashboard_pasti_7gg: 'Pasti (7gg)', dashboard_attenzione: 'Attenzione', dashboard_nessun_pasto: 'Nessun pasto',
    dashboard_vai: 'Vai', dashboard_giorni_fa: 'g fa', dashboard_ieri: 'ieri', dashboard_oggi: 'oggi',
    dashboard_tutto_regola: 'Tutto in regola!', dashboard_inizia: "Inizia l'allevamento",
    dashboard_aggiungi_primo: 'Aggiungi il primo serpente per cominciare.', dashboard_aggiungi_serpente: 'Aggiungi Serpente',
    serpenti_title: 'I Miei Serpenti', serpenti_count_suffix: 'esemplari registrati', serpenti_aggiungi: 'Aggiungi',
    serpenti_locked_singular: 'esemplare', serpenti_locked_plural: 'esemplari', serpenti_locked_suffix: 'in sola lettura',
    serpenti_locked_desc: 'Il piano Free consente la gestione attiva dei primi {N} serpenti inseriti. I dati degli altri restano salvati e consultabili.',
    serpenti_sblocca_pro: 'Sblocca con Pro', serpenti_empty_title: 'Nessun serpente',
    serpenti_empty_desc: 'Inizia aggiungendo il primo esemplare.', serpenti_specie: 'Specie', serpenti_eta: 'Età',
    serpenti_ultimo_pasto: 'Ultimo pasto', serpenti_ultime_feci: 'Ultime feci', serpenti_dettagli: 'Dettagli',
    serpenti_locked_tooltip: 'Sola lettura — passa a Pro per gestirlo',
    aggiungi_title: 'Nuovo Serpente', aggiungi_subtitle: 'Registra un nuovo esemplare',
    aggiungi_piano_free: 'Piano Free', aggiungi_serpenti_usati: 'serpenti usati',
    aggiungi_foto_opz: 'Foto (opzionale)', aggiungi_carica_foto: 'Carica foto',
    aggiungi_dati_anagrafici: 'Dati Anagrafici', aggiungi_nome: 'Nome *', aggiungi_specie: 'Specie',
    aggiungi_morfo: 'Morfo / Varietà', aggiungi_sesso: 'Sesso *', aggiungi_seleziona: '— Seleziona —',
    aggiungi_maschio: 'Maschio', aggiungi_femmina: 'Femmina', aggiungi_nascita: 'Data di nascita',
    aggiungi_peso: 'Peso attuale (g)', aggiungi_provenienza: 'Provenienza', aggiungi_icd: 'Codice ICD',
    aggiungi_note: 'Note', aggiungi_note_ph: 'Eventuali annotazioni...', aggiungi_salva: 'Salva', aggiungi_annulla: 'Annulla',
    aggiungi_ph_nome: 'es. Kaa', aggiungi_ph_specie: 'es. Python regius', aggiungi_ph_morfo: 'es. Albino, Pastel...',
    aggiungi_ph_peso: 'es. 450', aggiungi_ph_provenienza: 'es. Allevamento XYZ',
    err_nome_richiesto: 'Inserisci il nome!', err_sesso_richiesto: 'Seleziona il sesso!', err_nome_usato: 'già utilizzato',
    det_indietro: 'Indietro', det_pasti: 'Pasti', det_cibo_tot: 'Cibo tot.', det_feci: 'Feci', det_pulizie: 'Pulizie',
    det_mute: 'Mute', det_stampa: 'Stampa', det_vendi: 'Vendi',
    det_tab_pasti: 'Pasti', det_tab_feci: 'Feci', det_tab_uova: 'Uova', det_tab_pulizia: 'Pulizia', det_tab_mute: 'Mute', det_tab_peso: 'Peso', det_tab_info: 'Info',
    label_data: 'Data *', label_note: 'Note', label_storico: 'Storico', nessuna_registrazione: 'Nessuna registrazione',
    reg_pasto: 'Registra Pasto', tipo_cibo: 'Tipo di cibo', peso_preda: 'Peso preda (g)', quantita: 'Quantità',
    food_topo_sc: 'Topo scongelato', food_topo_vivo: 'Topo vivo', food_ratto_sc: 'Ratto scongelato', food_ratto_vivo: 'Ratto vivo',
    food_pulcino: 'Pulcino', food_coniglio: 'Coniglio', food_altro: 'Altro',
    ph_pasto_note: 'es. Ha mangiato con entusiasmo', salva_pasto: 'Salva Pasto',
    reg_feci: 'Registra Feci', consistenza: 'Consistenza',
    feci_normale: 'Normale', feci_liquide: 'Liquide', feci_dure: 'Dure', feci_urati: 'Con urati', feci_sangue: 'Sanguinolente',
    ph_feci_note: 'es. Aspetto normale',
    reg_deposizione: 'Registra Deposizione', n_uova: 'N° uova', n_fertili: 'N° fertili', temp_incubazione: 'Temp. incubaz. (°C)',
    ph_uova_note: 'es. Incubazione avviata',
    reg_pulizia: 'Registra Pulizia Vasca', tipo_pulizia: 'Tipo pulizia',
    pulizia_completa: 'Pulizia completa', pulizia_parziale: 'Pulizia parziale', cambio_acqua: 'Cambio acqua', disinfezione: 'Disinfezione',
    ph_pulizia_note: 'es. Sostituito substrato', storico_pulizie: 'Storico Pulizie', nessuna_pulizia: 'Nessuna pulizia registrata',
    reg_muta: 'Registra Muta', esito: 'Esito',
    muta_completa: 'Completa', muta_parziale: 'Parziale', muta_problematica: 'Problematica', muta_in_corso: 'In corso',
    ph_muta_note: 'es. Muta completa in un pezzo unico', salva_muta: 'Salva Muta', storico_mute: 'Storico Mute', nessuna_muta: 'Nessuna muta registrata',
    reg_peso: 'Registra Peso', peso_g: 'Peso (g) *', ph_peso_note: 'es. Post pasto, pre muta...', salva_peso: 'Salva Peso',
    curva_crescita: 'Curva di Crescita', servono_2_pesate: 'Servono almeno 2 pesate per il grafico',
    storico_pesate: 'Storico Pesate', nessuna_pesata: 'Nessuna pesata registrata',
    scheda_anagrafica: 'Scheda Anagrafica', modifica: 'Modifica', salva_modifiche: 'Salva Modifiche',
    lbl_nome: 'Nome', lbl_specie: 'Specie', lbl_morfo: 'Morfo', lbl_sesso: 'Sesso', lbl_nascita: 'Nascita',
    lbl_peso: 'Peso', lbl_provenienza: 'Provenienza', lbl_icd: 'Codice ICD', lbl_data_nascita: 'Data nascita',
    err_data_richiesta: 'Inserisci la data!',
    log_cibo_default: 'Cibo', log_uova_desc: 'uova', log_fertili_desc: 'fertili',
    registro_title: 'Registro', registro_ultime: 'Ultime {N} attività',
    registro_empty_title: 'Nessuna attività', registro_empty_desc: 'Le attività appariranno qui.',
    log_feci_prefix: 'Feci: ', log_deposizione_prefix: 'Deposizione: ', log_pulizia_prefix: 'Pulizia: ',
    venduti_title: 'Serpenti Venduti', venduti_count: 'esemplari venduti', venduti_incasso: 'Incasso totale:',
    venduti_empty_title: 'Nessuna vendita', venduti_empty_desc: "I serpenti venduti appariranno qui con tutto il loro storico.",
    venduti_badge: 'VENDUTO', venduti_data_vendita: 'Data vendita', venduti_acquirente: 'Acquirente', venduti_prezzo: 'Prezzo',
    err_elimina_venduto: 'Eliminare questo record dalla lista venduti?', eliminato: 'Eliminato',
    dv_venduto_il: 'Venduto il', dv_non_specificato: 'Non specificato', dv_giorni: 'Giorni',
    dv_storico_alimentazione: 'Storico Alimentazione', dv_nessun_pasto: 'Nessun pasto registrato',
    dv_storico_feci: 'Storico Feci', dv_storico_deposizioni: 'Storico Deposizioni', dv_nessuna_deposizione: 'Nessuna deposizione',
    dv_giorni_allevamento: 'Giorni in allevamento',
    profilo_title: 'Il mio profilo', profilo_subtitle: 'Gestisci il tuo account e abbonamento',
    profilo_account: 'Account', profilo_membro_dal: 'Membro dal',
    logo_title: 'Logo allevamento',
    logo_desc: "Se carichi un logo (immagine o PDF), verrà mostrato automaticamente sull'etichetta stampabile dei serpenti e sui PDF esportati. Il logo viene centrato automaticamente. Se non lo carichi, quelle sezioni restano semplicemente senza logo.",
    logo_cambia: 'Cambia logo', logo_carica_btn: 'Carica logo', logo_rimuovi: 'Rimuovi logo',
    piano_attuale: 'Piano attuale',
    piano_free_desc: 'Stai usando il piano gratuito. Puoi gestire fino a {N} serpenti.',
    piano_pro_desc: 'Hai accesso completo a tutte le funzionalità di SnakeKeeper Pro.',
    piano_admin_desc: 'Accesso amministratore completo — nessun limite.',
    piano_forever_desc: 'Hai accesso completo a SnakeKeeper gratuitamente per sempre.',
    passa_a_pro: 'Passa a Pro', serpenti_utilizzati: 'Serpenti utilizzati',
    limite_raggiunto: 'Hai raggiunto il limite — passa a Pro per continuare',
    serpenti_illimitati_lbl: 'Serpenti registrati:', serpenti_illimitati_suffix: '(illimitati)',
    pro_features_title: 'Cosa include Pro',
    pf1_t: 'Serpenti illimitati', pf1_d: 'Nessun limite al numero di esemplari',
    pf2_t: 'Statistiche avanzate', pf2_d: 'Grafici e analisi dettagliate',
    pf3_t: 'Export PDF & Excel', pf3_d: 'Esporta i tuoi dati',
    pf4_t: 'Backup prioritario', pf4_d: 'I tuoi dati sempre al sicuro',
    pf5_t: 'Supporto dedicato', pf5_d: 'Risposte prioritarie',
    pf6_t: 'Aggiornamenti gratuiti', pf6_d: 'Tutte le future funzionalità',
    price_mese: '€4.99/mese', price_anno: '€47.99/anno', price_forlife: 'For Life — €199 una tantum',
    gestione_abbonamento: 'Gestione abbonamento', caricamento_dettagli: 'Caricamento dettagli...',
    disdici_abbonamento: 'Disdici abbonamento',
    azioni_account: 'Azioni account', cambia_password: 'Cambia password', esci_account: "Esci dall'account",
    zona_pericolosa: 'Zona pericolosa',
    zona_pericolosa_desc: "Elimina definitivamente il tuo account e tutti i dati associati: serpenti, registri, archivio venduti e abbonamento. Questa azione non può essere annullata.",
    elimina_account_btn: 'Elimina account definitivamente',
    privacy_policy: 'Privacy Policy', termini_servizio: 'Termini di Servizio', gestisci_cookie: 'Gestisci Cookie',
    pdf_scheda_serpente: 'Scheda Serpente', pdf_doc_generato: 'Documento generato il',
    pdf_pasti_tot: 'Pasti tot.', pdf_pesate: 'Pesate', pdf_peso_attuale: 'Peso attuale',
    pdf_storico_pasti: 'Storico Pasti', pdf_storico_mute: 'Storico Mute', pdf_storico_peso: 'Storico Peso',
    pdf_storico_feci: 'Storico Feci', pdf_deposizioni: 'Deposizioni',
    pdf_col_data: 'Data', pdf_col_tipo: 'Tipo', pdf_col_peso: 'Peso', pdf_col_qty: 'Qty', pdf_col_note: 'Note',
    pdf_col_esito: 'Esito', pdf_col_uova: 'Uova', pdf_col_fertili: 'Fertili', pdf_col_temp: 'Temp',
    pdf_generato_footer: 'Generato:', pdf_no_popup: 'Abilita i popup per visualizzare il PDF',
    pdf_gestionale_sub: 'Gestionale Allevamento Serpenti', pdf_scheda_titolo_tab: 'Scheda',
    pdf_venduto_a: 'Venduto a', pdf_acquirente: 'Acquirente', pdf_prezzo_vendita: 'Prezzo di vendita',
    pdf_giorni_allevamento: 'Giorni in allevamento', pdf_riepilogo_vendita: 'Riepilogo Vendita',
    pdf_resoconto_vita: 'Resoconto Vita', pdf_scansiona_qr: 'Scansiona il QR per la scheda completa',
    pdf_resoconto_esemplare: 'Resoconto Esemplare', pdf_storico_completo: 'Storico completo vita in allevamento',
    pdf_stampa_salva: 'Stampa / Salva PDF', pdf_chiudi: 'Chiudi',
    pdf_peso_registrato: 'Peso registrato', pdf_ingresso_allevamento: 'Ingresso allevamento',
    pdf_riepilogo_attivita: 'Riepilogo Attività', pdf_pasti_totali: 'Pasti totali',
    pdf_cibo_somministrato: 'Cibo somministrato', pdf_feci_registrate: 'Feci registrate',
    pdf_giorni_tra_pasti: 'Giorni tra pasti', pdf_totale: 'Totale', pdf_uova_deposte: 'Uova deposte',
    pdf_temp_c: 'Temp. (°C)', pdf_dettagli_vendita: 'Dettagli Vendita', pdf_giorni_suffix: 'giorni',
    pdf_scansiona_apri: 'Scansiona per aprire la scheda',
    pdf_inquadra_camera: '📷 Inquadra con la fotocamera<br>per aprire su qualsiasi smartphone',
    pdf_scheda_esemplare: 'Scheda esemplare', pdf_ritaglia_prefix: '✂️ Ritaglia e applica sulla teca',
    pdf_nascita_short: 'Nascita',
    venduti_filtro_anno: 'Anno', venduti_tutti_anni: 'Tutti gli anni',
    venduti_filtro_mese: 'Mese', venduti_tutti_mesi: 'Tutti i mesi',
    venduti_reset_filtri: '✕ Rimuovi filtri', venduti_grafico_title: 'Andamento vendite',
    venduti_grafico_hint: '💡 Grafico interattivo — clicca le barre per applicare il filtro',
    venduti_vendite_label: 'vendite', venduti_nessuna_nel_periodo: 'Nessuna vendita in questo periodo.',
    foto_pos_label: 'Posizione foto',
    gen_title: 'Genealogia', gen_padre: 'Padre', gen_madre: 'Madre', gen_genetica: 'Genetica',
    gen_genetica_ph: 'es. het Pastel, Clown visivo', gen_nessuno: '— Nessuno —',
    gen_esterno_label: 'Esterno (non in archivio)', gen_esterno_ph: 'Nome esterno',
    gen_figli: 'Figli', gen_fratelli: 'Fratelli di cova', gen_nessun_figlio: 'Nessun figlio registrato',
    gen_nessun_fratello: 'Nessun fratello di cova', gen_partner: 'Partner (maschio)',
    gen_esterno_badge: 'esterno', gen_genitori: 'Genitori', gen_non_impostato: 'Non impostato',
    trial_badge_prefix: 'Pro gratis per', trial_badge_days: 'giorni', trial_badge_code: 'codice',
    trial_badge_expires: 'Scade il', trial_countdown_title: 'La tua prova gratuita sta per scadere',
    trial_countdown_desc_1: 'Il tuo periodo Pro gratuito con il codice', trial_countdown_desc_2: 'scade tra',
  ...
  en: {
    ...
    dashboard_totale: 'Total', dashboard_maschi: 'Males', dashboard_femmine: 'Females',
    dashboard_pasti_mese: 'Meals/month', dashboard_deposizioni: 'Clutches',
    dashboard_pasti_7gg: 'Meals (7d)', dashboard_attenzione: 'Attention', dashboard_nessun_pasto: 'No meal logged',
    dashboard_vai: 'Go', dashboard_giorni_fa: 'd ago', dashboard_ieri: 'yesterday', dashboard_oggi: 'today',
    dashboard_tutto_regola: 'All good!', dashboard_inizia: 'Start your collection',
    dashboard_aggiungi_primo: 'Add your first snake to get started.', dashboard_aggiungi_serpente: 'Add Snake',
    serpenti_title: 'My Snakes', serpenti_count_suffix: 'snakes registered', serpenti_aggiungi: 'Add',
    serpenti_locked_singular: 'snake', serpenti_locked_plural: 'snakes', serpenti_locked_suffix: 'in read-only mode',
    serpenti_locked_desc: 'The Free plan allows active management of the first {N} snakes added. Data for the others remains saved and viewable.',
    serpenti_sblocca_pro: 'Unlock with Pro', serpenti_empty_title: 'No snakes yet',
    serpenti_empty_desc: 'Start by adding your first snake.', serpenti_specie: 'Species', serpenti_eta: 'Age',
    serpenti_ultimo_pasto: 'Last meal', serpenti_ultime_feci: 'Last feces', serpenti_dettagli: 'Details',
    serpenti_locked_tooltip: 'Read-only — upgrade to Pro to manage it',
    aggiungi_title: 'New Snake', aggiungi_subtitle: 'Register a new animal',
    aggiungi_piano_free: 'Free plan', aggiungi_serpenti_usati: 'snakes used',
    aggiungi_foto_opz: 'Photo (optional)', aggiungi_carica_foto: 'Upload photo',
    aggiungi_dati_anagrafici: 'Basic Info', aggiungi_nome: 'Name *', aggiungi_specie: 'Species',
    aggiungi_morfo: 'Morph / Variety', aggiungi_sesso: 'Sex *', aggiungi_seleziona: '— Select —',
    aggiungi_maschio: 'Male', aggiungi_femmina: 'Female', aggiungi_nascita: 'Date of birth',
    aggiungi_peso: 'Current weight (g)', aggiungi_provenienza: 'Origin', aggiungi_icd: 'ICD Code',
    aggiungi_note: 'Notes', aggiungi_note_ph: 'Any notes...', aggiungi_salva: 'Save', aggiungi_annulla: 'Cancel',
    aggiungi_ph_nome: 'e.g. Kaa', aggiungi_ph_specie: 'e.g. Python regius', aggiungi_ph_morfo: 'e.g. Albino, Pastel...',
    aggiungi_ph_peso: 'e.g. 450', aggiungi_ph_provenienza: 'e.g. XYZ Breeder',
    err_nome_richiesto: 'Enter a name!', err_sesso_richiesto: 'Select a sex!', err_nome_usato: 'already in use',
    det_indietro: 'Back', det_pasti: 'Meals', det_cibo_tot: 'Total food', det_feci: 'Feces', det_pulizie: 'Cleanings',
    det_mute: 'Sheds', det_stampa: 'Print', det_vendi: 'Sell',
    det_tab_pasti: 'Meals', det_tab_feci: 'Feces', det_tab_uova: 'Eggs', det_tab_pulizia: 'Cleaning', det_tab_mute: 'Sheds', det_tab_peso: 'Weight', det_tab_info: 'Info',
    label_data: 'Date *', label_note: 'Notes', label_storico: 'History', nessuna_registrazione: 'No entries yet',
    reg_pasto: 'Log a Meal', tipo_cibo: 'Food type', peso_preda: 'Prey weight (g)', quantita: 'Quantity',
    food_topo_sc: 'Frozen/thawed mouse', food_topo_vivo: 'Live mouse', food_ratto_sc: 'Frozen/thawed rat', food_ratto_vivo: 'Live rat',
    food_pulcino: 'Chick', food_coniglio: 'Rabbit', food_altro: 'Other',
    ph_pasto_note: 'e.g. Ate enthusiastically', salva_pasto: 'Save Meal',
    reg_feci: 'Log Feces', consistenza: 'Consistency',
    feci_normale: 'Normal', feci_liquide: 'Loose', feci_dure: 'Hard', feci_urati: 'With urates', feci_sangue: 'Bloody',
    ph_feci_note: 'e.g. Normal appearance',
    reg_deposizione: 'Log a Clutch', n_uova: 'No. of eggs', n_fertili: 'No. fertile', temp_incubazione: 'Incubation temp. (°C)',
    ph_uova_note: 'e.g. Incubation started',
    reg_pulizia: 'Log Enclosure Cleaning', tipo_pulizia: 'Cleaning type',
    pulizia_completa: 'Full cleaning', pulizia_parziale: 'Partial cleaning', cambio_acqua: 'Water change', disinfezione: 'Disinfection',
    ph_pulizia_note: 'e.g. Substrate replaced', storico_pulizie: 'Cleaning History', nessuna_pulizia: 'No cleanings logged yet',
    reg_muta: 'Log a Shed', esito: 'Outcome',
    muta_completa: 'Complete', muta_parziale: 'Partial', muta_problematica: 'Problematic', muta_in_corso: 'In progress',
    ph_muta_note: 'e.g. Complete shed in one piece', salva_muta: 'Save Shed', storico_mute: 'Shed History', nessuna_muta: 'No sheds logged yet',
    reg_peso: 'Log Weight', peso_g: 'Weight (g) *', ph_peso_note: 'e.g. Post-meal, pre-shed...', salva_peso: 'Save Weight',
    curva_crescita: 'Growth Chart', servono_2_pesate: 'At least 2 weigh-ins are needed for the chart',
    storico_pesate: 'Weight History', nessuna_pesata: 'No weigh-ins logged yet',
    scheda_anagrafica: 'Profile Sheet', modifica: 'Edit', salva_modifiche: 'Save Changes',
    lbl_nome: 'Name', lbl_specie: 'Species', lbl_morfo: 'Morph', lbl_sesso: 'Sex', lbl_nascita: 'Born',
    lbl_peso: 'Weight', lbl_provenienza: 'Origin', lbl_icd: 'ICD Code', lbl_data_nascita: 'Date of birth',
    err_data_richiesta: 'Enter a date!',
    log_cibo_default: 'Food', log_uova_desc: 'eggs', log_fertili_desc: 'fertile',
    registro_title: 'Log', registro_ultime: 'Last {N} activities',
    registro_empty_title: 'No activity yet', registro_empty_desc: 'Activity will show up here.',
    log_feci_prefix: 'Feces: ', log_deposizione_prefix: 'Clutch: ', log_pulizia_prefix: 'Cleaning: ',
    venduti_title: 'Sold Snakes', venduti_count: 'snakes sold', venduti_incasso: 'Total revenue:',
    venduti_empty_title: 'No sales yet', venduti_empty_desc: 'Sold snakes will show up here with their full history.',
    venduti_badge: 'SOLD', venduti_data_vendita: 'Sale date', venduti_acquirente: 'Buyer', venduti_prezzo: 'Price',
    err_elimina_venduto: 'Delete this record from the sold list?', eliminato: 'Deleted',
    dv_venduto_il: 'Sold on', dv_non_specificato: 'Not specified', dv_giorni: 'Days',
    dv_storico_alimentazione: 'Feeding History', dv_nessun_pasto: 'No meals logged',
    dv_storico_feci: 'Feces History', dv_storico_deposizioni: 'Clutch History', dv_nessuna_deposizione: 'No clutches',
    dv_giorni_allevamento: 'Days in collection',
    profilo_title: 'My Profile', profilo_subtitle: 'Manage your account and subscription',
    profilo_account: 'Account', profilo_membro_dal: 'Member since',
    logo_title: 'Breeder Logo',
    logo_desc: "If you upload a logo (image or PDF), it will automatically appear on printable snake labels and exported PDFs. The logo is centered automatically. If you don't upload one, those sections simply stay without a logo.",
    logo_cambia: 'Change logo', logo_carica_btn: 'Upload logo', logo_rimuovi: 'Remove logo',
    piano_attuale: 'Current plan',
    piano_free_desc: 'You are on the Free plan. You can manage up to {N} snakes.',
    piano_pro_desc: 'You have full access to all SnakeKeeper Pro features.',
    piano_admin_desc: 'Full administrator access — no limits.',
    piano_forever_desc: 'You have full access to SnakeKeeper for free, forever.',
    passa_a_pro: 'Upgrade to Pro', serpenti_utilizzati: 'Snakes used',
    limite_raggiunto: "You've reached the limit — upgrade to Pro to continue",
    serpenti_illimitati_lbl: 'Snakes registered:', serpenti_illimitati_suffix: '(unlimited)',
    pro_features_title: "What's included in Pro",
    pf1_t: 'Unlimited snakes', pf1_d: 'No limit on the number of animals',
    pf2_t: 'Advanced statistics', pf2_d: 'Detailed charts and analytics',
    pf3_t: 'PDF & Excel export', pf3_d: 'Export your data',
    pf4_t: 'Priority backup', pf4_d: 'Your data always safe',
    pf5_t: 'Dedicated support', pf5_d: 'Priority responses',
    pf6_t: 'Free updates', pf6_d: 'All future features',
    price_mese: '€4.99/month', price_anno: '€47.99/year', price_forlife: 'For Life — €199 one-time',
    gestione_abbonamento: 'Manage subscription', caricamento_dettagli: 'Loading details...',
    disdici_abbonamento: 'Cancel subscription',
    azioni_account: 'Account actions', cambia_password: 'Change password', esci_account: 'Sign out',
    zona_pericolosa: 'Danger zone',
    zona_pericolosa_desc: 'Permanently delete your account and all associated data: snakes, logs, sold archive and subscription. This action cannot be undone.',
    elimina_account_btn: 'Permanently delete account',
    privacy_policy: 'Privacy Policy', termini_servizio: 'Terms of Service', gestisci_cookie: 'Manage Cookies',
    pdf_scheda_serpente: 'Snake Profile', pdf_doc_generato: 'Document generated on',
    pdf_pasti_tot: 'Total meals', pdf_pesate: 'Weigh-ins', pdf_peso_attuale: 'Current weight',
    pdf_storico_pasti: 'Feeding History', pdf_storico_mute: 'Shed History', pdf_storico_peso: 'Weight History',
    pdf_storico_feci: 'Feces History', pdf_deposizioni: 'Clutches',
    pdf_col_data: 'Date', pdf_col_tipo: 'Type', pdf_col_peso: 'Weight', pdf_col_qty: 'Qty', pdf_col_note: 'Notes',
    pdf_col_esito: 'Outcome', pdf_col_uova: 'Eggs', pdf_col_fertili: 'Fertile', pdf_col_temp: 'Temp',
    pdf_generato_footer: 'Generated:', pdf_no_popup: 'Please enable pop-ups to view the PDF',
    pdf_gestionale_sub: 'Snake Breeding Management', pdf_scheda_titolo_tab: 'Profile',
    pdf_venduto_a: 'Sold to', pdf_acquirente: 'Buyer', pdf_prezzo_vendita: 'Sale price',
    pdf_giorni_allevamento: 'Days in collection', pdf_riepilogo_vendita: 'Sale Summary',
    pdf_resoconto_vita: 'Life Summary', pdf_scansiona_qr: 'Scan the QR code for the full profile',
    pdf_resoconto_esemplare: 'Animal Report', pdf_storico_completo: 'Complete life history in the collection',
    pdf_stampa_salva: 'Print / Save PDF', pdf_chiudi: 'Close',
    pdf_peso_registrato: 'Recorded weight', pdf_ingresso_allevamento: 'Added to collection',
    pdf_riepilogo_attivita: 'Activity Summary', pdf_pasti_totali: 'Total meals',
    pdf_cibo_somministrato: 'Food given', pdf_feci_registrate: 'Feces logged',
    pdf_giorni_tra_pasti: 'Days between meals', pdf_totale: 'Total', pdf_uova_deposte: 'Eggs laid',
    pdf_temp_c: 'Temp. (°C)', pdf_dettagli_vendita: 'Sale Details', pdf_giorni_suffix: 'days',
    pdf_scansiona_apri: 'Scan to open the profile',
    pdf_inquadra_camera: '📷 Point your camera at it<br>to open on any smartphone',
    pdf_scheda_esemplare: 'Animal Profile', pdf_ritaglia_prefix: '✂️ Cut out and attach to the enclosure',
    pdf_nascita_short: 'Born',
    venduti_filtro_anno: 'Year', venduti_tutti_anni: 'All years',
    venduti_filtro_mese: 'Month', venduti_tutti_mesi: 'All months',
    venduti_reset_filtri: '✕ Clear filters', venduti_grafico_title: 'Sales trend',
    venduti_grafico_hint: '💡 Interactive chart — click a bar to apply the filter',
    venduti_vendite_label: 'sales', venduti_nessuna_nel_periodo: 'No sales in this period.',
    foto_pos_label: 'Photo position',
    gen_title: 'Genealogy', gen_padre: 'Father', gen_madre: 'Mother', gen_genetica: 'Genetics',
    gen_genetica_ph: 'e.g. het Pastel, visual Clown', gen_nessuno: '— None —',
    gen_esterno_label: 'External (not in your records)', gen_esterno_ph: 'External name',
    gen_figli: 'Offspring', gen_fratelli: 'Clutch siblings', gen_nessun_figlio: 'No offspring registered',
    gen_nessun_fratello: 'No clutch siblings', gen_partner: 'Partner (male)',
    gen_esterno_badge: 'external', gen_genitori: 'Parents', gen_non_impostato: 'Not set',
    trial_badge_prefix: 'Free Pro for', trial_badge_days: 'days', trial_badge_code: 'code',
    trial_badge_expires: 'Expires on', trial_countdown_title: 'Your free trial is ending soon',
    trial_countdown_desc_1: 'Your free Pro period with code', trial_countdown_desc_2: 'expires in',
...
function applyAppLang() {
  document.querySelectorAll('#mobile-drawer [data-it], aside [data-it], #mobile-nav [data-it]').forEach(el => {
...
function showLoginScreen() {
  ...
  if (btnLogin) { btnLogin.disabled = false; btnLogin.innerHTML = '🔓 Accedi'; }
  const btnRegister = document.getElementById('btn-register');
  if (btnRegister) { btnRegister.disabled = false; btnRegister.innerHTML = '🐍 Crea Account'; }
}
...
function showAuthMsg(msg, isError=false) {
  ...
  el.style.background = isError ? 'rgba(192,57,43,0.15)' : 'rgba(109,181,109,0.15)';
  el.style.color = isError ? 'var(--accent-red)' : 'var(--accent-lime)';
  el.style.border = isError ? '1px solid rgba(192,57,43,0.3)' : '1px solid rgba(109,181,109,0.3)';
}

async function doLogin() {
  const email = document.getElementById('login-email').value.trim();
  const password = document.getElementById('login-password').value;
  if (!email || !password) { showAuthMsg('Inserisci email e password', true); return; }
  ...
  try {
    ...
    btn.disabled = false; btn.innerHTML = '🔓 Accedi';
  }
}
...
function selectRegLang(lang) {
  _regLang = lang;
  document.getElementById('reg-lang-it').classList.toggle('active', lang === 'it');
  document.getElementById('reg-lang-en').classList.toggle('active', lang === 'en');
}
...
async function doRegister() {
  ...
  if (!email || !password) { showAuthMsg('Inserisci email e password', true); return; }
  if (password.length < 6) { showAuthMsg('La password deve avere almeno 6 caratteri', true); return; }
  const btn = document.getElementById('btn-register');
  btn.disabled = true; btn.innerHTML = '<span class="spinner"></span> Registrazione...';
  try {
    await authReq('signup', { email, password, data: { lang: _regLang } });
    showAuthMsg("✅ Registrazione completata! Controlla SUBITO la tua email (anche lo spam) e clicca il link per verificare l'account. Il link scade dopo un po', quindi verificalo appena possibile.");
    btn.disabled = false; btn.innerHTML = '🐍 Crea Account';
  } catch(e) {
    showAuthMsg('❌ ' + e.message, true);
    btn.disabled = false; btn.innerHTML = '🐍 Crea Account';
  }
...
async function doForgotPassword() {
  const email = document.getElementById('login-email').value.trim();
  if (!email) { showAuthMsg('Inserisci prima la tua email', true); return; }
  try {
...
function showDeleteAccountModal() {
  ...
  modal.style.cssText = 'position:fixed;inset:0;z-index:700;background:rgba(8,15,9,0.92);display:flex;align-items:center;justify-content:center;padding:24px';
  modal.innerHTML = `
...
async function doDeleteAccount() {
  ...
  try {
    const _authSession = await getSession();
    if (!_authSession) throw new Error('Sessione non valida, effettua di nuovo il login.');
    ...
    const data = await res.json();
    if (!res.ok || data.error) throw new Error(data.error || 'Errore durante l\'eliminazione');
...
function showExpiredLinkModal(errorCode, errorDesc) {
  ...
  modal.style.cssText = 'position:fixed;inset:0;z-index:800;background:rgba(8,15,9,0.95);display:flex;align-items:center;justify-content:center;padding:24px';
  modal.innerHTML = `
    <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:16px;padding:32px;max-width:400px;width:100%;text-align:center;box-shadow:var(--shadow)">
  ...
  try {
    ...
    if (params.access_token && params.type) {
      ...
      try {
        ...
        console.warn('Recupero utente completo dopo link email fallito:', sessErr);
      }
